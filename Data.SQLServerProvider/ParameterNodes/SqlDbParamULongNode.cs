...
{
    public class SqlDbParamULongNode : DbParameterNode
    {
        public SqlDbParamULongNode(IDbParameterParser parser) : base(parser) 
        {
        }
        ...
        public override IDbDataParameter GetDbParameter()
        {
            ...
            if (MapPropertyType != typeof(ulong))
            {
                if (Next != null)
                    return Next.GetDbParameter();
            }

            var dbType = DbType.Int64; // SQL Server Implementation.
            var size = 8;
